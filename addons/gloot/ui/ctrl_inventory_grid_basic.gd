...
func _ready() -> void:
	...
	var activate_func = func(dragable: CtrlDragable, grab_position: Vector2):
		_ctrl_drop_zone.activate()
	CtrlDragable.dragable_grabbed.connect(activate_func)
	var deactivate_func = func(dragable: CtrlDragable, zone: CtrlDropZone, drop_position: Vector2):
		_ctrl_drop_zone.deactivate()
	CtrlDragable.dragable_dropped.connect(deactivate_func)
	_ctrl_drop_zone.tree_exiting.connect(func():
		CtrlDragable.dragable_grabbed.disconnect(activate_func)
		CtrlDragable.dragable_dropped.disconnect(deactivate_func)
	)
