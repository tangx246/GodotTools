...
func split_stack_safe(item: InventoryItem, new_stack_size: int) -> InventoryItem:
	...
	# Check if there's space for the new item before splitting
	# This prevents the original item from being modified when the split would fail
	var space_for_item := _get_space_for_single_item(inventory, item)
	if space_for_item.le(ItemCount.zero()):
		return null
...
func transfer_automerge(item: InventoryItem, destination: Inventory) -> bool:
	...
	for i in destination.get_items():
		...
		if item.is_queued_for_deletion():
			...
			return true
	# Check if grid has space before calling add_item to prevent infinite recursion
	# (add_item calls transfer_automerge when grid is full but stacks have space)
	var gc := destination._constraint_manager.get_grid_constraint()
	if gc != null and not gc.has_space_for(item):
		return false
